...
def load_problem_categories(file_path="categories.json"):
    if os.path.exists(file_path):
        ...
        raise FileNotFoundError(f"{file_path} not found. Please generate it first.")

def select_random_problem_type():
    categories = load_problem_categories()
    academic_types = categories["academic"]
    real_world_types = categories["real_world"]

    # Randomly select a category and a problem type from each
    selected_academic_category = random.choice(list(academic_types.keys()))
    selected_academic_type = random.choice(academic_types[selected_academic_category])

    selected_real_world_category = random.choice(list(real_world_types.keys()))
    selected_real_world_type = random.choice(real_world_types[selected_real_world_category])

    return {
        "academic_category": selected_academic_category,
        "academic_type": selected_academic_type,
        "real_world_category": selected_real_world_category,
        "real_world_type": selected_real_world_type
    }
...
def generate_ip_prompts(problem_type_info):
    academic_type = problem_type_info["academic_type"]
    real_world_type = problem_type_info["real_world_type"]

    return {
        "description": (
            f"Write a natural language description of a {academic_type} multivariate integer complex programming problem in the context of {real_world_type}. "
        ...
        "data_json": (
            f"Create a JSON file containing all the essential data parameters for a {academic_type} multivariate integer complex programming problem in the context of {real_world_type} "
        ...
        "input_json": (
            f"Create a JSON file that defines the parameters for a {academic_type} multivariate integer complex programming problem in the context of {real_world_type}. "
        ...
        "input_targets_json": (
            f"Provide a machine-readable representation of a {academic_type} multivariate integer complex programming problem in the context of {real_world_type} in JSON format. "
...
async def batch_generate_multiple_sets_async(problem_type_info, batch_count=3, log_file="generation_status.log"):
    ...
    for batch_index in range(batch_count):
        ...
        if batch_key not in generation_status:
            generation_status[batch_key] = {
                ...
                "problem_type_json": False
            }

        prompts_dict = generate_ip_prompts(problem_type_info)
        ...
        output_files = [
            ...
            os.path.join(batch_folder, "problem_type.json")
        ]

        if not generation_status[batch_key]["problem_type_json"]:
                    with open(output_files[4], "w", encoding="utf-8") as file:
                        json.dump(problem_type_info, file, indent=4, ensure_ascii=False)
                    print(f"{output_files[4]} saved successfully.")
                    generation_status[batch_key]["problem_type_json"] = True
...
if __name__ == "__main__":
    import nest_asyncio
    nest_asyncio.apply()
    problem_type_info = select_random_problem_type()
    loop = asyncio.get_event_loop()
    if loop.is_running():
        task = loop.create_task(batch_generate_multiple_sets_async(problem_type_info, batch_count=1))
        loop.run_until_complete(task)
    else:
        asyncio.run(batch_generate_multiple_sets_async(problem_type_info, batch_count=1))
